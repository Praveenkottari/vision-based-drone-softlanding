...
## Aruco marker
An ArUco marker is a synthetic square marker composed by a wide black border and Inner binary matrix that encodes the marker's ID and that is used extensively in computer vision applications, especially for pose estimation and augmented reality tasks. The size of this matrix can vary, commonly being 4x4, 5x5, 6x6, or 7x7. The binary pattern is what makes each marker unique. The matrix consists of black and white squares, where black typically represents a '0' and white represents a '1’. In inner matrix odd columns are represents the parity bits even columns contain data bits. Their simplicity and robustness to various lighting conditions and perspectives make them particularly useful for real-time applications. The maximum number of markers that can be encoded is 22n where n-number of data Colum.
